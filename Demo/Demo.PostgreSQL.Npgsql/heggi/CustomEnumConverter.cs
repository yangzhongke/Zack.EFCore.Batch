...
{
    private static readonly Expression<Func<TEnum, string>> toStringExpr = x => ToDbString(x);
    private static readonly Expression<Func<string, TEnum>> toEnumExpr = x => ToEnum(x);
    ...
    public static TEnum ToEnum(string value)
    {
        foreach (var name in Enum.GetNames<TEnum>())
        {
            ...
            if (attr != null && attr.Value == value) return Enum.Parse<TEnum>(name);
        }
