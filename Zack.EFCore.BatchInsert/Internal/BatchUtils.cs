...
public static class BatchUtils
{
    ...
    public static string GetPKColName<TEntity>(DbSet<TEntity> dbSet) where TEntity : class
    {
        ...
        var pkColName = pkProp.GetColumnName(StoreObjectIdentifier.SqlQuery(dbSet.EntityType));
        return pkColName;
    }

    private static Expression<Func<TEntity, object>> GetSelectPkExpression<TEntity>(DbContext dbCtx)
        where TEntity : class
    {
        var parameter = Expression.Parameter(typeof(TEntity), "e");
        var pkProp = GetPKProperty(dbCtx.Set<TEntity>());
        var pkPropName = pkProp.Name;

        var body = Expression.Convert(Expression.MakeMemberAccess(parameter, typeof(TEntity).GetProperty(pkPropName)),
            typeof(object));
        return (Expression<Func<TEntity, object>>)Expression.Lambda(body, parameter);
    }

    public static string BuildWhereSubQuery<TEntity>(IQueryable<TEntity> queryable, DbContext dbCtx,
        string aliasSeparator) where TEntity : class
    {
        var queryProvider = queryable.Provider;
        ...
        var queryingEnumerable =
            queryProvider.Execute<IEnumerable>(whereQuerable.Expression) as IRelationalQueryingEnumerable;
        ...
        var tableAlias = "temp1";
        var dbSet = dbCtx.Set<TEntity>();
        var pkName = GetPKColName(dbSet);
        var sqlGenHelpr = dbCtx.GetService<ISqlGenerationHelper>();
        var quotedPkName = sqlGenHelpr.DelimitIdentifier(pkName); //pkId-->"pdId" on NPgsql
    ...
    {
        if (!newSQL.StartsWith(oldSQL)) throw new ArgumentException("!newSQL.StartsWith(oldSQL)", nameof(newSQL));
        return newSQL.Substring(oldSQL.Length);
    }

    //this method is from source code ef core
    public static void GenerateList<T>(IReadOnlyList<T> items, IRelationalCommandBuilder Sql,
        Action<T> generationAction, Action<IRelationalCommandBuilder> joinAction = null)
    {
        if (joinAction == null)
    ...
    {
        if (selectExpression.Offset == null && selectExpression.Limit == null && !selectExpression.IsDistinct &&
            selectExpression.Predicate == null && selectExpression.Having == null &&
            selectExpression.Orderings.Count == 0 && selectExpression.GroupBy.Count == 0 &&
            selectExpression.Tables.Count == 1)
        {
            var tableExpressionBase = selectExpression.Tables[0];
            var setOperation = tableExpressionBase as SetOperationBase;
    ...
    {
        if (conn.State != ConnectionState.Open) return conn.OpenAsync(cancellationToken);

        return Task.CompletedTask;
    }

    public static IDictionary<string, object> ConvertParameterValues(this DbContext ctx,
        IReadOnlyDictionary<string, object> modelValues)
    {
    ...
    {
        if (modelObject == null) return DBNull.Value;
        var mp = typeMappingSource.FindMapping(modelObject.GetType());
        if (mp == null || mp.Converter == null || mp.Converter.ConvertToProvider == null) return modelObject;
        var providerValue = mp.Converter.ConvertToProvider(modelObject);
        if (providerValue == null) return DBNull.Value;
