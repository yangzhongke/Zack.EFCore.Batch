...
namespace Zack.EFCore.BatchInsert.Npgsql;
...
public class NpgSqlBulkInsertExecutor : IBulkInsertExecutor
{
    ...
    private static string BuildDestTableName(IEntityType entityType, ISqlGenerationHelper sqlGenHelpr)
    {
        ...
        if (string.IsNullOrWhiteSpace(schemaName)) return sqlGenHelpr.DelimitIdentifier(tableName);
        ...
        Type entityType, IEnumerable items)
    {
        ...
        var destinationTableName = BuildDestTableName(efEntityType, sqlGenHelpr);
        var dataTable = BulkInsertUtils.BuildDataTable(dbCtx, efEntityType, entityType, items);
        var colNames = dataTable.Columns.OfType<DataColumn>()
            .Select(c => c.ColumnName).ToArray();
        var delimitedCols = string.Join(",", colNames.Select(n => sqlGenHelpr.DelimitIdentifier(n)));
        var writer =
            pgConn.BeginBinaryImport($"COPY {destinationTableName} ({delimitedCols}) FROM STDIN (FORMAT BINARY)");
